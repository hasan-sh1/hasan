...
async def send_scheduled_messages():
    ...
    now = datetime.now()
    if now.minute == 0 and now.hour in scheduled_messages:  # التحقق من الساعة والدقيقة
        try:
            channel = bot.get_channel(channel_id)
            if channel:
                await channel.send(scheduled_messages[now.hour], allowed_mentions=discord.AllowedMentions.none())
        except Exception as e:
