...
scheduled_messages = {
    (3,40):"اللهم إني أسألك الهدى والتقى والعفاف والغنى ",
...
async def send_scheduled_messages():
    ...
    now = datetime.now()
    current_time = (now.hour, now.minute)  # الحصول على الوقت الحالي كساعة ودقيقة
    if current_time in scheduled_messages:  # التحقق إذا كان الوقت الحالي موجود في التذكيرات
        try:
            channel = bot.get_channel(channel_id)
            if channel:
